    ...
    button.addEventListener("click", async () => {
      ...
      if (action === "Generate meeting summary") {
        ...
        try {
          ...
          alert(
            "Error starting summary generation: " + (error.message || error),
          );
  ...
  if (generateButton) {
    generateButton.addEventListener("click", async () => {
      ...
      try {
        ...
        sdkLogger.log(
          "Auto button: Requesting AI summary generation for meeting: " +
            currentEditingMeetingId,
        );

        const result = await window.electronAPI.generateMeetingSummaryStreaming(
          currentEditingMeetingId,
        );
